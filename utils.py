...
def train(train_loader, encoder, decoder, criterion, optimizer, vocab_size,
    ...
    for i_step in range(start_step, total_step + 1):
        ...
        if i_step % PRINT_EVERY == 0:
            print("\r" + stats)
            filename = os.path.join("/home/osboxes/image_captioning/example", "train-model-{}{}.pkl".format(epoch, i_step))
...
def validate(val_loader, encoder, decoder, criterion, vocab, epoch, 
    ...
    with torch.no_grad():
        for i_step in range(start_step, total_step + 1):
            ...
            if i_step % PRINT_EVERY == 0:
                print("\r" + stats)
                filename = os.path.join("/home/osboxes/image_captioning/example", "val-model-{}{}.pkl".format(epoch, i_step))
...
#     PIL_image = Image.open('/home/osboxes/Infinity10/images/women.jpg').convert("RGB")
...
#     image = transform_test(PIL_image)
    ...
    if torch.cuda.is_available():
        image = image.cuda()
    features = encoder(image).unsqueeze_(1)
